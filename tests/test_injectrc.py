...
INJECTRC = "home/bin/gremlins/executable_injectrc"
EXPECTED_REGEN = """
...
def test_dryrun(cli):
    ...
    assert "## -- Added by injectrc -- DO NOT MODIFY THIS SECTION" in cli.logged.stderr
...
def test_samples(cli):
    ...
    cli.run("-v", "samples/bashrc", "file:samples/ensure-path", "-m", "my-test-app")
    assert cli.succeeded
    assert "Section has not changed, not modifying 'samples/bashrc'" in cli.logged.stderr.contents()

    cli.run("-n", "samples/bashrc", "_empty_", "-m", "my-test-app")
    assert cli.succeeded
